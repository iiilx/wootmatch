...
function switchForm() {
    $("#autocomplete").toggle();
    $("#normal").toggle();
    var num_friends = window.globaldata.length;
    var select = document.getElementById("friends");
    for (var i=0; i < num_friends; i++) {
        var option = document.createElement("option");
        option.innerHTML = window.globaldata[i];
        option.value = window.fids[i];
...
function showAbout() {
  $("#about_link").hide();
  $("#about").slideToggle(); //slideDown(); //show();
   
};

function hideAbout() { 
   $("#about").hide();
   $("#about_link").slideToggle(); //show();
};
...
$("#contact").qtip({
    content: 'Please e-mail wootmatch@gmail.com with any questions!', 
    show: 'mouseover',
    hide: 'mouseout',
    style: 'mystyle2', 
    position: {
...
var split_url = url.split(/\/+/g);
var path = split_url[2];
if (path == 'request-features') {
 $("#rf").toggleClass("activated", true);
}
else if (path == '') {$("#mylikes").toggleClass("activated", true);}

/*$("#contact").qtip({
    content: 'Please e-mail likeeachother@gmail.com with any questions!', 
    show: 'mouseover',
    hide: 'mouseout',
    style: 'mystyle', 
    position: {
...
function showDelete() {
    $("#tos").hide();
    $("#privacy").hide();
    $("#delete-account").slideDown(); //show();
};

function addLikeNormal() {
    var uid = $("#friends").val();
    var name = window.globaldata[window.fids.indexOf(uid)];
    addLike(name,uid);
};

function addLikeAuto() {
  var name = document.getElementById("example").value;
  var uid = window.fids[window.globaldata.indexOf(name)];
    addLike(name,uid);
};

function addLike(name,uid) {
    ...
    if(name != ""){
        ...
        if ($(rowId).length > 0) {
            ...
            $.ajax({
                type: "POST",
                data: ({ 'add_like_uid' : uid, 'add_like_name': name, 'rank': $('tr').length}),
                url: '/add-person',
                success: function(response){
                    if (response == 'too_many') {
                        alert('5 people is the maximum.');
                        }
                    else {
                        var row = $(response);
                        $("#tbl_container").show();
...
function updateMatches() {
    $.ajax({
        ...
        error: function(){ 
            alert('Could not update matches.');
        }
    });
